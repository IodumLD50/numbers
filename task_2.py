...
#input - string, а string является списком символа.
# Приобразовываем помученное значение в int.
dozens = int(number[3]) # Десятки.
units = int(number[4]) # Единицы.
hundreds = int(number[2]) # Сотни.
tens_of_thousands = int(number[0]) # Десятки тысечных.
thousand = int(number[1]) # Тысячные.

decision = dozens ** units * hundreds / (tens_of_thousands - thousand)
print(f"Ответ: {decision}")